...
export default function DashboardPage() {
  ...
  // Phases from onboarding (localStorage)
  const [onboardingPhases, setOnboardingPhases] = useState<{ label: string; target: number; type: string }[] | null>(null);
  useEffect(() => {
    try {
      const raw = localStorage.getItem('onboardingData');
      if (raw) {
        const data = JSON.parse(raw);
        if (Array.isArray(data.phases) && data.phases.length > 0) setOnboardingPhases(data.phases);
      }
    } catch { /* ignore */ }
  }, []);
  const {
        ...
        <div className={styles.zoneInner}>
          ...
          <div className={styles.headerDaily}>
            <PrimaryGoalHeroWidget
              ...
              variant="header"
              phaseLabel={(() => {
                if (!onboardingPhases || !summary.primaryGoal) return null;
                const current = summary.primaryGoal.currentAmount;
                let idx = -1;
                for (let i = 0; i < onboardingPhases.length; i++) {
                  if (current < onboardingPhases[i].target) { idx = i; break; }
                }
                if (idx === -1) return `🏆 Meta alcanzada`;
                return `Fase ${idx + 1} de ${onboardingPhases.length} · ${onboardingPhases[idx].label}`;
              })()}
            />
          </div>
        </div>
